...
type CreatorCandidate = { id: string; name: string; handle: string; platform: string; url: string; audience: string; fit: number; evidence: string; niche: string };
function CreatorDiscovery({ onAdd }: { onAdd: (candidate: CreatorCandidate) => void }) {
  ...
  return <section className="creator-discovery"><div className="form-head"><div><p className="eyebrow">LIVE CREATOR DISCOVERY</p><h3>Search for people who already have the audience.</h3></div><span className="status"><Search size={14} /> Exa-powered</span></div><div className="creator-search"><label>Profit pocket or niche<input value={niche} onChange={(event) => setNiche(event.target.value)} placeholder="e.g. newborn care for first-time mothers" /></label><button className="primary" onClick={run} disabled={state === "running"}>{state === "running" ? "Searching creators…" : "Find creators"}</button></div>{state !== "idle" && <div className={state === "error" ? "creator-search-status error" : "creator-search-status"}><Search size={16} /> {message}</div>}{state === "complete" && <div className="creator-candidates">{creators.map((creator) => <article key={creator.id}><span>{creator.platform}</span><h4>{creator.name}</h4><a href={creator.url} target="_blank" rel="noreferrer">{creator.handle}</a><p>{creator.evidence}</p><div><strong>{creator.fit}</strong><small>fit signal</small></div><button className={added.includes(creator.id) ? "secondary candidate-added" : "secondary"} disabled={added.includes(creator.id)} onClick={() => add(creator)}>{added.includes(creator.id) ? <><Check size={14} /> Added</> : <><Plus size={14} /> Add to network</>}</button></article>)}</div>}</section>;
}
