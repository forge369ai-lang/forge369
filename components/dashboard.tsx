...
function CreatorNetwork() {
  ...
  function addDiscoveredCreator(candidate: CreatorCandidate) {
    setCreators((current) => current.some((creator) => creator.handle.toLowerCase() === candidate.handle.toLowerCase()) ? current : [{ name: candidate.name, handle: candidate.handle, niche: candidate.niche, audience: candidate.audience, fit: candidate.fit, stage: "Prospect" }, ...current]);
  }
  return (
    <section className="creator-network">
      ...
      <CreatorDiscovery onAdd={addDiscoveredCreator} />
...
type CreatorCandidate = { id: string; name: string; handle: string; platform: string; url: string; followers: number | null; followerLabel: string; countSource: string; audience: string; fit: number; evidence: string; inMicroRange: boolean; niche: string };
function CreatorDiscovery({ onAdd }: { onAdd: (candidate: CreatorCandidate) => void }) {
  const [niche, setNiche] = useState("");
  const [state, setState] = useState<"idle" | "running" | "complete" | "error">("idle");
  const [message, setMessage] = useState("");
  const [creators, setCreators] = useState<CreatorCandidate[]>([]);
  const [added, setAdded] = useState<string[]>([]);
  async function run() { setState("running"); setMessage(""); try { const response = await fetch("/api/creators", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ niche }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error || "Creator search failed."); const found = (data.creators || []).map((creator: Omit<CreatorCandidate, "niche">) => ({ ...creator, niche: niche || "Unassigned" })); setCreators(found); setAdded([]); setMessage(`${data.sourceCount} Instagram profiles found. ${data.verifiedMicroCount} are in the 10k–100k target range from available public counts.`); setState("complete"); } catch (error) { setMessage(error instanceof Error ? error.message : "Creator search failed."); setState("error"); } }
  function add(candidate: CreatorCandidate) { if (added.includes(candidate.id)) return; onAdd(candidate); setAdded((current) => [...current, candidate.id]); }
  return <section className="creator-discovery"><div className="form-head"><div><p className="eyebrow">LIVE CREATOR DISCOVERY</p><h3>Search actual Instagram profiles in the micro-creator range.</h3></div><span className="status"><Search size={14} /> Forge intelligence</span></div><div className="creator-search"><label>Profit pocket or niche<input value={niche} onChange={(event) => setNiche(event.target.value)} placeholder="e.g. newborn care for first-time mothers" /></label><button className="primary" onClick={run} disabled={state === "running"}>{state === "running" ? "Enriching profiles…" : "Find Instagram creators"}</button></div>{state !== "idle" && <div className={state === "error" ? "creator-search-status error" : "creator-search-status"}><Search size={16} /> {message}</div>}{state === "complete" && <div className="creator-candidates">{creators.map((creator) => <article key={creator.id}><span>{creator.platform}</span><h4>{creator.name}</h4><a href={creator.url} target="_blank" rel="noreferrer">{creator.handle}</a><div className={creator.inMicroRange ? "creator-followers qualified" : "creator-followers"}><strong>{creator.followerLabel}</strong><small>followers · {creator.inMicroRange ? "target range" : creator.countSource}</small></div><p>{creator.evidence}</p><div><strong>{creator.fit}</strong><small>fit signal</small></div><button className={added.includes(creator.id) ? "secondary candidate-added" : "secondary"} disabled={added.includes(creator.id) || !creator.inMicroRange} onClick={() => add(creator)}>{added.includes(creator.id) ? <><Check size={14} /> Added</> : creator.inMicroRange ? <><Plus size={14} /> Add to network</> : "Not qualified yet"}</button></article>)}</div>}</section>;
}
