...
export default function Dashboard() {
  ...
  return <main className="shell"><aside className="sidebar"><div className="brand"><span className="brand-mark"><Flame size={19} /></span><span>FORGE<span>369</span></span></div><p className="eyebrow">DIGITAL PRODUCT FOUNDRY</p><nav>{nav.map(([label, Icon]) => <button onClick={() => setView(label)} className={view === label ? "nav-item active" : "nav-item"} key={label}><Icon size={17} /> {label}{label === "Opportunities" && <span className="count">{ranked.length}</span>}</button>)}</nav><div className="sidebar-bottom"><button className="nav-item"><Settings size={17} /> Settings</button><div className="profile"><div className="avatar">JS</div><div><strong>Jay Steenkamp</strong><span>Founder workspace</span></div></div></div></aside>
    <section className="workspace"><header><div><p className="eyebrow">FORGE369 MVP · DEMO MODE</p><h1>{view}</h1><p>Find demand. Forge value. Ship products.</p></div><button className="primary" onClick={runScan}><Sparkles size={17} /> Run discovery scan</button></header>
      ...
      {view === "Product Studio" && <ProductStudio active={active} charterReady={charterReady} setCharterReady={setCharterReady} onPackage={() => setView("Launch Packs")} onAssets={() => setView("Product Assets")} />}
      {view === "Product Assets" && <ProductAssets databaseState={databaseState} />}
...
function ProductAssets({ databaseState }: { databaseState: "checking" | "not-configured" | "signed-out" | "ready" | "saving" }) {
  const [selectedAsset, setSelectedAsset] = useState(1);
  const asset = lucidAssets[selectedAsset];
  const secure = databaseState === "ready";
  return <section className="asset-vault"><div className="asset-hero"><div><p className="eyebrow">PRODUCT ASSETS · THE LUCID THRESHOLD</p><h2>Your complete customer package</h2><p>One polished offer, eleven customer-facing assets, and a single delivery bundle prepared for release.</p><div className="asset-stats"><span><strong>11</strong> assets</span><span><strong>13.2 MB</strong> package</span><span><strong>v1.0</strong> release</span></div></div><div className="vault-mark"><FolderLock size={27} /><span>Private vault</span></div></div><div className="secure-notice"><FolderLock size={18} /><div><strong>{secure ? "Private asset vault connected" : "Assets staged for secure storage"}</strong><span>{secure ? "Files can now be uploaded to your private Supabase bucket and delivered with controlled access." : "Your package is intentionally not published to this public repository or a public Vercel URL. Sign in when your email access is restored to upload it privately."}</span></div></div><div className="asset-layout"><div className="asset-list"><div className="asset-list-head"><span>Customer deliverables</span><small>{lucidAssets.length} files</small></div>{lucidAssets.map(([name, label, meta], index) => <button className={selectedAsset === index ? "asset-row selected" : "asset-row"} onClick={() => setSelectedAsset(index)} key={name}>{name.endsWith(".mp3") ? <FileAudio size={18} /> : <FileText size={18} />}<div><strong>{label}</strong><span>{meta}</span></div><Check size={15} /></button>)}</div><aside className="asset-preview"><p className="eyebrow">ASSET PREVIEW</p>{asset[0].endsWith(".mp3") ? <FileAudio size={38} /> : <FileText size={38} />}<h3>{asset[1]}</h3><p>{asset[0]}</p><span className="asset-ready"><Check size={14} /> Staged and quality-checked</span><button className="secondary full-width"><Eye size={15} /> Preview after secure upload</button><button className="primary full-width" disabled={!secure}><Download size={15} /> {secure ? "Download secure asset" : "Secure download pending sign-in"}</button></aside></div></section>;
}
